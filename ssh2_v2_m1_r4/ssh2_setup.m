...
%             % marianne wrote this awhile ago but may not need it any more
%             fprintf('Adding Ganymed-ssh2 to the java path by running\n%javaaddpath(''%s'')\n',ganymed_java_library_jar_path);
%             save_globals_addpath(ganymed_java_library_jar_path);
...
global javaaddpathstaticflag
jcp = javaclasspath('-all');
gany_found = 0;
for jcp_index = 1:length(jcp)
    if size(strfind(jcp{jcp_index},ssh2_struct.ganymed_java_library_jar),1) && exist(jcp{jcp_index},'file')
        gany_found = gany_found + 1;
    end
end
if (gany_found > 0) || javaaddpathstaticflag
