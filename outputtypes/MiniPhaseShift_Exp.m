function h=MiniPhaseShift_Exp(cells,Hz,a1,varargin)
...
if isempty(a1)
if Hz<20
    h=figure('Color','w','Units','inches','Name','Exp Theta LFP','PaperUnits','inches','PaperPosition',[0 0 9 7]); % ,'FontName','Verdana','FontWeight','Bold','FontSize',16
else
    h=figure('Color','w','Units','inches','Name','Exp Gamma LFP','PaperUnits','inches','PaperPosition',[0 0 9 7]); % ,'FontName','Verdana','FontWeight','Bold','FontSize',16
end
pos = get(h,'Position');
...
for n=1:length(cells)
    ...
    if ~isempty(midx)
        cells(n).phase=nrninput(midx).phase;
        ...
        distIDX(n)=0;
    elseif strcmp(cells(n).name,'scacell')==1
