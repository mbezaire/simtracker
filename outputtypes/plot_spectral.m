function [h, varargout]=plot_spectral(handles,varargin)
%PLOT_SPECTRAL  Plot spectral analysis of simulation results.
%   H = PLOT_SPECTRAL(HANDLES) where HANDLES contains a field called CURSES
%   and CURSES contains a field called spikerast will plot the spectral
%   analysis (pwelch, periodogram, or FFT) of the network's spike density 
%   function (SDF), spike times, LFP, or MP overall or by cell type.
%   
%       h = plot_spectral(handles);
%   
%   returns the handle of the newly generated figure, h, for the plot.
%
%   H = PLOT_SPECTRAL(HANDLES) where HANDLES contains a field OPTARG where
%   handles.optarg = TYPE
...
artflag=0;
if ~isempty(strfind(tmp,'art'))
    artflag=1;
end

datarange=[2 100];
if ~isempty(strfind(tmp,'['))
    myrangestr=regexp(tmp,'\[[0-9\s]+\]','match');
    tmp=strrep(tmp,myrangestr{:},'');
...
if nargin>1 && ~isempty(varargin)
    handles.curses.spikerast(handles.curses.spikerast(:,1)>varargin{1},:)=[];
    handles.runinfo.SimDuration=varargin{1};
end
...
if strcmp(datatype,'lfp')
    ...
    fint=find(f>=datarange(1) & f<=datarange(2));
    f=f(fint);
    fft=fft(fint);
    switch dispformat
        case 'table'
            ...
            [~, gamma_idx] = max(fft(gamma_range));
            rel_range=find(f(:)>datarange(1) & f(:)<=datarange(2));
            [~, over_idx] = max(fft(rel_range));
            tbldata{r,1}='LFP';
            tbldata{r,2}=f(theta_range(peak_idx));
            tbldata{r,3}=fft(theta_range(peak_idx));
            tbldata{r,4}=f(gamma_range(gamma_idx));
            tbldata{r,5}=fft(gamma_range(gamma_idx));
            tbldata{r,6}=f(rel_range(over_idx));
            tbldata{r,7}=fft(rel_range(over_idx));
...
switch org
    case 'type'
        ridx=0;
        for r=1:length(handles.curses.cells)
            if strcmp(handles.curses.cells(r).techname(1:2),'pp')==0 || artflag==1
                if posflag==1
...
switch dispformat
    ...
    case 'heat'
        ...
        cbar_axes=colorbar;
        if normflag
            set(get(cbar_axes,'ylabel'),'string','Norm. Power')
        else
            set(get(cbar_axes,'ylabel'),'string','Power')
        end
        set(gca,'ytick',[1:2/(length(sety)-1):3],'yticklabel',fliplr(sety))
    ...
    case 'table'
        ...
        for r=1:length(myresults)
            peak_idx=[];gamma_idx=[];over_idx=[];
            theta_range=find(myresults(r).f(:)>=5 & myresults(r).f(:)<=10);
            [~, peak_idx] = max(myresults(r).fft(theta_range));
            gamma_range=find(myresults(r).f(:)>=25 & myresults(r).f(:)<=80);
            [~, gamma_idx] = max(myresults(r).fft(gamma_range));
            ...
            tbldata{r,4}=myresults(r).f(gamma_range(gamma_idx));
            tbldata{r,5}=myresults(r).fft(gamma_range(gamma_idx));
            tbldata{r,6}=myresults(r).f(rel_range(over_idx));
            tbldata{r,7}=myresults(r).fft(rel_range(over_idx));
        end
...
tableh = uitable(h, 'Data', tbldata,'ColumnFormat',{'char','bank','bank','bank','bank'},'ColumnName',{[tblstr ': Name'],'Theta (Hz)','Theta Power','Gamma (Hz)','Gamma Power','Peak (Hz)','Peak Power'},'Units','inches', 'UIContextMenu',mycontextmenuz);
...
if ~isempty(strfind(tmp,'sdf')) || isempty(tmp) || (isempty(strfind(tmp,'spike')) && isempty(strfind(tmp,'lfp')) && isempty(strfind(tmp,'mp')))
    ...
    if ~isempty(strfind(tmp,'fft'))
        if timeflag
            [f, fft_results]=mycontfft([handles.general.crop:1:timewindow],sdf);
        else
...
if ~isempty(strfind(tmp,'spike'))
    ...
    if ~isempty(strfind(tmp,'fft'))
        if timeflag
            [f, fft_results]=myfft(spikes,timewindow,rez,handles.general.crop);
        else
            [f, fft_results]=myfft(spikes,RunArray(ind).SimDuration,rez,handles.general.crop);
        end
    elseif ~isempty(strfind(tmp,'gram'))
        Fs = 1000/rez; % sampling frequency (per s)
        if timeflag
            bins=[0:rez:timewindow];
        else
