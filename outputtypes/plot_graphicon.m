...
for lm=1:length(levellist)
    ...
    %             celltype(z).output.(post) = celltype(z).output.(post)/celltype(m).numcells;
    %         end
    %     end
    % end
    ...
    for ztmp=1:length(abbrev.(level))
        ...
        leveldata(z).numcells = sum([celltype(boutonidx(zidx)).numcells]);
        try
    ...
    [prefields, ipre] = intersect({leveldata(:).(level)},unique({celltype(boutonidx).(level)}),'stable');
    ...
    for r=1:length(leveldata)
        ...
        for z=1:length(postfields)
            ...
            if remainingbouts>0 % sideways
                leveldata(r).h(hi) = rectangle('Position',[0 cellheight cellwidth+synwidth remainingbouts],'EdgeColor',colorvec.(level)(tc,:),'FaceColor',colorvec.(level)(tc,:));
            end
            ...
            if r==length(leveldata) %r==length(leveldata)
                try
    ...
    annotation('textarrow','Position',yaxpos,'String','Interneuron-Innervating Boutons','Color',[1 1 1],'TextColor',[0 0 0],'TextEdgeColor',[1 1 1],'FontName','Verdana','TextRotation',90,'HorizontalAlignment','left','VerticalAlignment','bottom')
    annotation('textarrow','Position',xaxpos,'String','Inhibitory Input Synapses','Color',[1 1 1],'TextColor',[0 0 0],'TextEdgeColor',[1 1 1],'FontName','Verdana','VerticalAlignment','middle','HorizontalAlignment','right')

    set(gcf,'ButtonDownFcn',{@displaystuff, leveldata, level,colorvec,abbrev,name,bfilters,sfilters,mfilters,margin,separater,'con'})
    ch=get(gcf,'Children');
    for c=1:length(ch)
        set(ch(c),'ButtonDownFcn',{@displaystuff, leveldata, level,colorvec,abbrev,name,bfilters,sfilters,mfilters,margin,separater,'con'})
        ch2=get(ch(c),'Children');
        for h=1:length(ch2)
            set(ch2(h),'ButtonDownFcn',{@displaystuff, leveldata, level,colorvec,abbrev,name,bfilters,sfilters,mfilters,margin,separater,'con'})
        end
    end
    print([pathway 'conv' levellist{lm} '.png'],'-dpng')
    print([pathway 'conv' levellist{lm} '.eps'],'-depsc')
end