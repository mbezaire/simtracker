function h=MiniPhaseShift_Figure(cells,Hz,a1,varargin)
...
if isempty(a1)
    try
if Hz<20
    h=figure('GraphicsSmoothing','off', 'Renderer', 'painters','Color','w','Units','inches','Name','Mini Theta LFP','PaperUnits','inches','PaperPosition',[0 0 1.75 1]); % ,'FontName','Verdana','FontWeight','Bold','FontSize',16
    %h=figure('Color','w','Units','inches','Name','Mini Theta LFP','PaperUnits','inches','PaperPosition',[0 0 9 7]); % ,'FontName','Verdana','FontWeight','Bold','FontSize',16
else
    h=figure('GraphicsSmoothing','off', 'Renderer', 'painters','Color','w','Units','inches','Name','Mini Gamma LFP','PaperUnits','inches','PaperPosition',[0 0 1.75 1]); % ,'FontName','Verdana','FontWeight','Bold','FontSize',16
end
    catch
if Hz<20
    h=figure('Renderer', 'painters','Color','w','Units','inches','Name','Mini Theta LFP','PaperUnits','inches','PaperPosition',[0 0 1.75 1]); % ,'FontName','Verdana','FontWeight','Bold','FontSize',16
    %h=figure('Color','w','Units','inches','Name','Mini Theta LFP','PaperUnits','inches','PaperPosition',[0 0 9 7]); % ,'FontName','Verdana','FontWeight','Bold','FontSize',16
else
    h=figure('Renderer', 'painters','Color','w','Units','inches','Name','Mini Gamma LFP','PaperUnits','inches','PaperPosition',[0 0 1.75 1]); % ,'FontName','Verdana','FontWeight','Bold','FontSize',16
end
    end
pos = get(h,'Position');
set(h,'Position',[.7 .7  1.75 1]);
...
for n=1:length(cells)
    ...
    if isempty(midx) & strcmp(cells(n).name,'scacell')==1
        midx = strmatch(matchnames.nice{strmatch('cckcell',matchnames.tech)},{nrninput(:).name});
    end
...
for ntmp=1:length(distIDX)
    ...
    plot(trace.data,excellTMP,'LineWidth',1.5,'Marker','none','Color',colvec);
    %plot(trace.data,excellTMP,'LineWidth',4,'Marker','none','Color',colvec);
    hold on
...
%     set(gk,'Color',colvec,'HorizontalAlignment','right','FontName','ArialMT','FontWeight','Bold','FontSize',14);
    ...
    if distIDX(n)==0
        %plot(trace.data(enIDX(n)),excellTMP(enIDX(n)),'Marker','o','MarkerEdgeColor',colvec,'MarkerFaceColor',colvec,'MarkerSize',15);
        plot(trace.data(enIDX(n)),excellTMP(enIDX(n)),'Marker','o','MarkerEdgeColor',colvec,'MarkerFaceColor',colvec,'MarkerSize',5);
        continue;
    end
    %plot(trace.data(stIDX(n):enIDX(n)),excellTMP(stIDX(n):enIDX(n)),'LineWidth',6,'Marker','none','Color',colvec);
    plot(trace.data(stIDX(n):enIDX(n)),excellTMP(stIDX(n):enIDX(n)),'LineWidth',2,'Marker','none','Color',colvec);
    if arrowDir(n)==1
        lidx=find(distlist<=trace.data(enIDX(n)),1,'last');
        mrkstr=mrklistEN{lidx};
        %plot(trace.data(enIDX(n)),excellTMP(enIDX(n)),'Marker',mrkstr,'MarkerEdgeColor',colvec,'MarkerFaceColor',colvec,'MarkerSize',15);
        plot(trace.data(enIDX(n)),excellTMP(enIDX(n)),'Marker',mrkstr,'MarkerEdgeColor',colvec,'MarkerFaceColor',colvec,'MarkerSize',5);
    else
        lidx=find(distlist<=trace.data(stIDX(n)),1,'last');
        mrkstr=mrklistST{lidx};
        %plot(trace.data(stIDX(n)),excellTMP(stIDX(n)),'Marker',mrkstr,'MarkerEdgeColor',colvec,'MarkerFaceColor',colvec,'MarkerSize',15);
        plot(trace.data(stIDX(n)),excellTMP(stIDX(n)),'Marker',mrkstr,'MarkerEdgeColor',colvec,'MarkerFaceColor',colvec,'MarkerSize',5);
    end
end

axes(a1)
pp=get(gca,'Position');
%set(gca,'Position',[.05 pp(2) .9 pp(4)])
