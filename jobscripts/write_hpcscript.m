function write_hpcmpiscript(fid,fid2,jobtype,handles,m)
global RunArray
ind = handles.curses.ind;

fprintf(fid, '#!/bin/bash\n');
fprintf(fid, '#\n');
fprintf(fid, '#$ -q som,asom,pub64\n'); %, jobtype.queue);
fprintf(fid, '#$ -pe mpi %g\n', jobtype.cores);
...
fprintf(fid, 'mpiexec -np $CORES nrniv -mpi -nobanner -nogui ./jobscripts/%s_run.hoc\n', RunArray(ind).RunName); % Run the hoc program file
