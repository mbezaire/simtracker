...
% J. Aumentado
% 4/20/05
% Note: This is a global mypath change of font in the file. That is, any text you
...
%                                           %to test.eps.
figname=strrep(figname(~isspace(figname)),'''','');
figfilestr = [figname '.eps'];
disp(figname)
tic
% txtstruct=exporttext2latex(fignum,figname);
...
if exist('draftqual','var') && ~isempty(draftqual) && draftqual
    eval(['print -depsc2 -r120 -opengl -f' num2str(fignum) ' ' figfilestr ';']);
else
    eval(['print -depsc2 -r300 -opengl -f' num2str(fignum) ' ' figfilestr ';']);
    %eval(['print -depsc2 -r200 -f' num2str(fignum) ' ' figfilestr ';']);
end
eval(['print -dpng -f' num2str(fignum) ' ' [figname '.png'] ';']);
