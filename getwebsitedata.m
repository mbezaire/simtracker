function getwebsitedata(pathway,repotype,varargin) % channel % phys syns % exp syns
if ispc
    sl='\';
else
    sl='/';
...
if ~isempty(varargin)
    if ~isempty(varargin{1})
        ...
        if exist([repospath sl 'cellclamp_results' sl 'website' sl 'channels.dat'],'file')==0
            ...
            cmdstr=['cd ' repospath handles.dl ...
                ' ' handles.general.neuron ' -c mytstop=1100 -c duration=1000 -c starttime=50 -c cellmechs=1 ' ...
                '-c "strdef resultspath" -c resultspath="\"cellclamp_results' sl 'website\"" -c NumData=' num2str(NumData) ' -c ConnData=' num2str(ConnData) ' -c SynData=' num2str(SynData) ' ' ...
                ' -c "strdef toolpath" -c toolpath="\"' mypath sl 'tools\"" ' ...
                '-c "strdef relpath" -c relpath="\"' repospath '\"" -c "strdef sl" -c sl="\"/ \"" ' ...
                ' .' sl 'setupfiles' sl 'clamp' sl 'electrophys.hoc -c "quit()"'];            
            system(cmdstr);
        end
...
%             end
        ...
        if ~isempty(idx) && ~isempty(synidx) && isfield(allexptraces, pretype) && isfield(allexptraces.(pretype), posttype)
            ...
            if ischar(allexptraces.(pretype).(posttype).Reversal) && strcmp(allexptraces.(pretype).(posttype).Reversal,'auto')
                if isnan(synfiledata{8}(synidx(1)))
                    allexppscs.(pretype).(posttype).Reversal = [synfiledata{10}(synidx(1))]; % synfiledata{14}(synidx(1))]; % will need to update this when adding all poss synapse types... and mult reversal potentials
                else
                    allexppscs.(pretype).(posttype).Reversal = synfiledata{7}(synidx(1)); % will need to update this when adding all poss synapse types
                end
            else
...
print_website(pathway,allcells,allpscs,allchans,allephys,allconns,alltraces,allexppscs)
