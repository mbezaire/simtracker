...
%%%%%%%%%%%%%%%%%%%%%%%%%%Initial Conditions%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%V(1) = -.054;
% H-current fast acting gating variable
%mH_Fast = 1/((1 + (exp((V(1) + .10)/.003)))); %^(1.36)); 
%mH_Fast = 1/((1 + (exp((V(1) + .0728)/.008)))); %^(1.36)); 
%  = 0.08706
% H-current slow acting gating variable
%mH_Slow = 1/((1 + (exp((V(1) + .00283)/.0159)))^(58.5)); 
%  = 0.1007
% NaP activation gating variable
%m = 1/(1 + exp(-(V(1) + .0487)/.0044));  
%  = 0.23067
% NaP inactivation gating variable
%h = 1/(1 + exp((V(1) + .0488)/.00998)); 
%  = 0.6274
...
for i = 1:ctr-1
  ...
  alpha_activation_NaP = (.091e6*(V(i) + .038))/(1 - exp(-(V(i) + .038)/.005)); 
  beta_activation_NaP = (-.062e6*(V(i) + .038))/(1 - exp((V(i) + .038)/.005)); 
  %Persistant Na inactivation 
  alpha_inactivation_NaP = (-2.88*V(i) - .0491)/(1 - exp((V(i) - .0491)/.00463));
  ...
  mH_Fast_inf = 1/((1 + (exp((V(i) + .10)/.003)))); %^(1.36)); 
                                                    
  % H current gating variable steady state activation - slow component
  mH_Slow_inf = 1/((1 + (exp((V(i) + .00283)/.0159)))^(58.5)); 

  % H current time constant - fast component
  tauH_Fast = .00051/(exp((V(i) - .0017)/.010) + exp(-(V(i) + .34)/.52)); 

  % H current time constant - slow component
  tauH_Slow = .0056/(exp((V(i) - .017)/.014) + exp(-(V(i) + .26)/.043)); 
