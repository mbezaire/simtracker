...
function [p t y I] = mec2sc_scm_syn(varargin)
...
% final simulation
  ...
  if p.t_init
    ...
    if isfield(p,'ramp'), p0 = rmfield(p,'ramp'); end
    if isfield(p,'gsyn'), p0 = rmfield(p,'gsyn'); end
    p0.t_end = p.t_init;
    [t y s] = simulate(y0, p0);
    if spiked(), return, end
    y0 = y(end,:);
  end
  [t y s I] = simulate(y0, p);
  %%
  function b = spiked
    b = ~p.allow_spiking;
    if b
      b = ((~isempty(s) && isfield(s, 'xe') && length(s.xe)) || ...
           max(y(:,1)) > p.spike_thld);
    end
  end
end

function [t y s I] = simulate(y0, p)
...
% integration solution object
  ...
  if use_ode
    ...
    if size(tspan,2) > 2
      if ~p.allow_spiking && (r > 0 || length(s.xe))
        t = 0:p.dt:s.xe(1);
      else
        t = tspan;
      end
  ...
  % currents at evaluated time steps
  if nargout > 3
    I = intg_current(y, p);
    if isfield(p,'gsyn')
      I(:,end+1) = synaptic_current([],y(:,1),p);
    end
    y_end.I = I(end,:)
  end
end
...
% and currents vs. Vm in a single figure, returning graphic handle
  sd = get(0,'ScreenSize');
  set(0,'DefaultTextFontSize', 8, 'DefaultAxesFontSize', 7, ...
  ...
  % ion currents
  c = { 'I_H' 'I_{NaP}' 'I_{Na}' 'I_K' 'I_M' 'I_S' 'I_{Syn}' };
  subplot(7,3,3);
...
% for subthreshold oscillation model using H, NaP and M and spiking currents
  ...
  p.vdr = 'mec2sc_vdr(p)';
  p.gsyn_par = default_synapse_params();
  p.gsyn_fcn = @gsyn;
  p.solver = 'ode15s';
  p = dft_spiking(p);
...
% ion currents 
  ...
  % synaptic conductance profile
  if isfield(p,'gsyn_par')
    T = 0:p.dt:1000/p.gsyn_par.stim_reps.theta;
    p.gsyn = p.gsyn_fcn(T, p.gsyn_par);
  end
end
...
% p -- parameters (structure with fields as required below)
  ...
  k = p.state_key;
  if isfield(p,'is_ventral') && p.is_ventral
    cell_pos = 1;
  else
    cell_pos = 0;
  end
  ...
  % difference equations
  % membrane potential
  dy(k.V,1) = (I_inj - sum([I_syn I I_L])) / p.C_m;
...
% return integrated currents array
  ...
  if isfield(p, 'syn') && p.cell_area > 1e-8
    for i = 1:size(I,1)
...
% generates time course of conductance change for a single synaptic pulse
% can be passed to the Matlab pulstran function
function g = gsyn(t, p)
% t -- time vector
% p -- parameters
  % old Wilson and Bower form: gs_max*(taoD*taoR)/(taoD-taoR)
  scale1 = (p.taoD * p.taoR)/(p.taoD - p.taoR); 
  gscaled = p.gs_max / ((p.taoR/p.taoD)^(scale1/p.taoD) - ...
                       (p.taoR/p.taoD)^(scale1/p.taoR));
  g = zeros(size(t));
  for ti = 1:length(g)
    g(ti) = (exp(-t(ti)/p.taoD) - exp(-t(ti)/p.taoR)) * gscaled;
  end
end

% find total synaptic current obtained from pulse trains
function I = synaptic_current(ti, V, p)
% ti -- time index, for table lookup of synapse conductance
% V -- voltage (mV)
% p -- struct of required parameters
  psyn = p.gsyn_par;
  np = psyn.stim_reps.count;
  % time steps between pulses
  gi = ceil(1e3/(p.dt*psyn.stim_reps.gamma));
  % time steps between trains ("epoch")
  ei = ceil(1e3/(p.dt*psyn.stim_reps.theta));
  I = zeros(length(V),1);
  Ip = zeros(1,np);
  if isempty(ti) || ~ti, ti = 1:length(V); end
  for r = 1:length(I)
    % time step relative to current epoch
    tr = mod(ti(r),ei);
    for k = 1:np
      tl = tr - (k-1)*gi;
      if tl > 0
        Ip(k) = p.gsyn(tl) * V(r) / psyn.syn_area;
      end
    end
    I(r) = sum(Ip);
  end
...
% structure 
  ...
  if ~isfield(p,'spike_shift'), p.spike_shift = -50; end
  ...
  function tau = tau_n(V, m)
    if nargin > 1
      if m > 0, tau = tau_n_v(V); else tau = tau_n_d(V); end
    else
      p = chnl_par.H.tau_fast;
  ...
  function inf = n_inf(V, m)
    if nargin > 1
      if m > 0, inf = n_inf_v(V); else inf = n_inf_d(V); end
    else
  ...
  function tau = tau_k(V, m)
    if nargin > 1
      if m > 0, tau = tau_k_v(V); else tau = tau_k_d(V); end
    else
  ...
  function inf = k_inf(V, m)
    if nargin > 1
      if m > 0, inf = k_inf_v(V); else inf = k_inf_d(V); end
    else
...
function p = default_synapse_params()
  p = struct();
  p.dur = 100;  % msec duration of a pulse
  p.stim_reps = struct('theta',10,'gamma',80,'count',4);
  % from Garden et al. 2008
  p.V_L = -70;
  p.taoR = 0.2; % rise time constant in milliseconds 
  p.taoD = 2.5; % decay time constant in milliseconds
  p.gs_max = 1.8e-9; % maximal synaptic conductance, Siemens
  p.syn_area = 1e-4; % 0.0001 cm^2 in area
  p.Rm = 20e6; % typ. dorsal cell (1e6 = M Ohms)
end
